...
export class LetterFrequenciesENStrategy extends SolverStrategy {

  // We want the words that have the most higher count of most used letters in English
  override executeStrategy(solver: WordleSolver, possibleWords: Set<string>): Set<string> {
    ...
    for (const word of possibleWords) {
      const count = word.replaceAll('[^EARIO]', '').length;
